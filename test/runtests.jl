...
@testset "Bakhvalov_Vasil'eva_analytical" begin
    ...
    @test compute_integral([1], Δt = 3600, r = 1)               ≈ step_response(3600, 1)              atol = 5*10^-14
    @test compute_integral([1], Δt = 3600, r = 0.1)             ≈ step_response(3600, 0.1)            atol = 5*10^-14
    @test compute_integral([1; zeros(5)], Δt = 3600, r = 0.1)   ≈ short_step_response(5, 3600, 0.1)   atol = 5*10^-14
end

@testset "Bakhvalov_Vasil'eva_convolution" begin
    @test compute_integral([1], Δt = 3600, r = 1) - FiniteLineSource.convolve_step([1], Δt = 3600, r = 1)                         ≈ 0   atol = 5*10^-14
    @test compute_integral([1], Δt = 3600, r = 0.1) - FiniteLineSource.convolve_step([1], Δt = 3600, r = 0.1)                     ≈ 0   atol = 5*10^-14
