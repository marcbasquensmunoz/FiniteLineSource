...
block = @time prepare_containers(SegmentToPoint(D=0., H=0., z=0., σ=0.), bh_positions, ϵ, Nt, constants, nmodel);
...
# Precomputation
block = @btime prepare_containers(SegmentToPoint(D=0., H=0., z=0., σ=0.), bh_positions, ϵ, Nt, constants, nmodel);
...
Profile.clear()
@profile prepare_containers_ltp(bh_positions, ϵ, Nt, constants, nmodel);
pprof()