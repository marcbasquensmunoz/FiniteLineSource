...
function create_error_plot(ϵ_range, r̃_range, res_step, res_synth; xlabel=:sigma, title, rb)
    CairoMakie.activate!()
    fig = Figure()
    ...
    legend = Legend(fig, [legend_colors, legend_markers], [error_strings, ["Unit step", "Synthetic"]], [L"\frac{\epsilon}{Q}", "Load applied"], tellheight = false, tellwidth = true)
